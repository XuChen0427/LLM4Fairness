...
async def process_api_requests_from_file(
    ...
    # logging.debug(f"Logging initialized at level {logging.INFO}")
        ...
        while True:
            ...
            if next_request is None:
                if queue_of_requests_to_retry.empty() is False:
                    ...
                    try:
                        # get new request
                        content = eval(next(requests))
                        task_id, pos_target, request_json = content["task_id"], content[
                            "pos_target"], content["request"]
    ...
    async def call_API(
        ...
        try:
            ...
            #exit(0)
                ...
                if 'gpt-3.5-turbo' in response['model']:
                    ...
                    #exit(0)
                    if len(answer) < 9:
                        error = "Not fit output format, retry!"
                        print("id:"+str(self.task_id)+":",error)
                else:
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                append_to_jsonl([{"task_id": self.task_id}, self.request_json, self.result, {"pos_target":self.pos_target}], save_filepath)
                status_tracker.num_tasks_in_progress -= 1
                status_tracker.num_tasks_failed += 1
        else:
            append_to_jsonl([{"task_id": self.task_id}, self.request_json, response, {"pos_target":self.pos_target}], save_filepath)
            status_tracker.num_tasks_in_progress -= 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_requests_per_minute", type=int, default=3500 * 0.5)
    parser.add_argument("--max_tokens_per_minute", type=int, default=90000 * 0.5)
